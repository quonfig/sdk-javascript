import { TelemetryRequestError, type TelemetryHttpResult } from "./uploader";

/**
 * Telemetry transport policy, browser subset (qfg-y8je.11; policy P1-P10 in
 * project/plans/2026-09-24-sdk-telemetry-transport-policy.md, contract tests
 * in integration-test-data/chaos/telemetry-transport-contract.md).
 *
 * Mirrors sdk-node's src/telemetry/transportQueue.ts. This module owns the
 * retained queue of serialized batches, the send gate (30s floor after a
 * failure + Retry-After), the drain loop, disable-on-auth and the P7 logging
 * episodes. It knows nothing about the aggregator or payload shape: it stores
 * and resends opaque strings. Retention is in memory for the life of the page.
 */

/** Shipped defaults for the `telemetry*` init options (browser SDK class). */
...
/** pagehide / close() give the live window one POST with this deadline (P8, browser). */
export const SHUTDOWN_FLUSH_DEADLINE_MS = 2_000;

/** The four levels the transport logs at (P7). */
...
/** UTF-8 byte length of a serialized batch (what goes on the wire). */
...
export class TelemetryTransportQueue {
  ...
  append(body: string): void {
    ...
    const oversize = bytes > this.maxRetainedBytes;
    this.queue.push({ body, bytes, createdAt: Date.now(), oversize });
    ...
    while (count > this.maxRetainedBatches || total > this.maxRetainedBytes) {
   ...
   * failure. Tick step 5. Never rejects.
   */
  drain(): Promise<void> {
    const run = this.drainLoop().catch((err) =>
      this.logger.debug(`Telemetry drain failed: ${describeError(err)}`)
    );
    this.idle = run;
    return run;
  }
  ...
  /**
   * pagehide / close(): one POST of the live window bounded by `deadlineMs`,
   * sent with `keepalive` so it can outlive the page. Never retains, never
   * touches the outage episode, never throws. Does not take the one-in-flight
   * slot: on pagehide the page is going away and the regular POST, if any, is
   * already accounted for.
   */
  async sendFinal(body: string, deadlineMs: number): Promise<void> {
    let result: string | undefined;
    try {
      const res = await this.send(body, deadlineMs, new AbortController().signal, true);
    ...
    if (result !== undefined) {
      this.logger.debug(
        `Telemetry final flush at shutdown failed (${result}); ${byteLength(body)} bytes dropped, ${this.retainedCount} retained batch(es) abandoned`
      );
    }
  }
  ...
  private onSuccess(): void {
    ...
    const now = Date.now();
    const seconds = Math.round((now - (this.firstFailureAt ?? now)) / 1000);
    this.logger.info(
  ...
  private disable(status: number): void {
    ...
    this.logger.error(
      `Telemetry disabled for this page: ${this.telemetryUrl} answered ${status} (${hint}). Flag evaluation is unaffected.`
    );
    this.queue.length = 0;
...
function describeError(err: unknown): string {
  if (err instanceof TelemetryRequestError) {
    ...
    return `network error: ${cause instanceof Error ? cause.message : String(cause ?? err.message)}`;
  }
